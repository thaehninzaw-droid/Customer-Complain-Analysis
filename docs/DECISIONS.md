...
## Decision 34 — Banking pivot: dataset, categories, form, and business rules

**Date:** August 2026  
**Status:** Implemented  
**Author:** Advisor revision notes, implemented by the thesis team

### Context

The advisor reviewed the system and required the following changes before final submission:

1. The dataset must be banking complaints with long narrative text (not Comcast telecom tickets ~2–3 sentences each).
2. The customer complaint form must not expose a category selector or any "AI detected category" label to the customer.
3. Priority must be kept (Low / Medium / High). A new business rule is required: same user filing a similar complaint more than once on the same day → High.
4. The chatbot must remain, but must be grounded in SOP documents only — not trained on the complaint dataset.

### Decisions Made

#### A. Dataset — CFPB Consumer Complaints

We use the publicly available CFPB Consumer Complaint Database (Kaggle mirror: `sebastienverpile/consumercomplaintsdata`). This dataset contains 903,983 rows covering banking, credit card, mortgage, debt collection, and credit reporting complaints. Consumer complaint narratives are long, paragraph-length, real-world text exactly as the advisor requested.

**Cleaning pipeline** (`backend/data/clean_banking_dataset.py`):
- Raw file kept untouched at `backend/data/raw/consumer_complaints.csv`
- Only rows with a narrative ≥ 200 characters are kept (77.9 % of rows have no narrative at all)
- CFPB anonymisation tokens (`XXXX`) replaced with `[REDACTED]`
- Exact duplicate narratives dropped
- Each CFPB `Product` mapped to one of 5 banking categories (see table below)
- Dominant classes undersampled to 3,000 rows each (final: 12,000 rows, 3,000 per category)
- Output: `backend/data/banking_complaints.csv` (the ONLY file used by training and seeding)

Full EDA documented in `backend/data/EDA.md`.

#### B. Category mapping

| New Category | CFPB `Product` values |
|---|---|
| Cards | Credit card · Prepaid card · Credit card or prepaid card |
| Accounts | Checking or savings account · Bank account or service · Money transfers · Money transfer, virtual currency, or money service |
...
| Other banking | (default for any unmapped product) |

#### C. ML retraining

`train_classifier.py` was updated to use the official CFPB-mapped `category` column from `banking_complaints.csv` as training labels (not distant supervision). Accuracy on held-out CFPB-mapped labels: **86.5 %**.

`train_priority.py` was updated to use the banking CSV. Priority labels are still generated by distant supervision from `predict_priority_baseline()` (the CFPB dataset has no official priority column). The accuracy figure in `priority_metrics.json` measures the trained tree against the heuristic it learned from, not against human-labeled ground truth — this is documented in `priority_metrics.json` under `honesty_note`.

#### D. Customer form — category removed

The customer-facing complaint form (`frontend/activities.html`) no longer contains a category `<select>` or any "AI detected category" display. Category is always classified server-side by Algorithm 1 after submission. The `POST /complaints` endpoint ignores any `category` field the client might send (the Pydantic schema no longer includes it in `ComplaintIn`).

The customer-facing complaint history table shows: Ticket # / Complaint / Date / Priority / Status / Actions — no category column.

#### E. Same-user same-day repeat rule

New business rule added server-side in `backend/app/main.py`:

```
_is_repeat_same_day(user_id, new_text, new_date, existing_complaints)
    → bool
```

If user X already filed at least one complaint on the same calendar date AND the new complaint has Jaccard word-set overlap ≥ 0.70 (or is an exact text match after normalisation) with any of those prior complaints, the priority of the new complaint is forced to **High** regardless of the model's prediction.

This rule runs server-side and cannot be bypassed via the frontend. Jaccard is computed on lowercased word sets (not character n-grams) — fast, interpretable, and sufficient for detecting resubmissions.

#### F. Chatbot — SOP grounding

The chatbot's `RECOMMENDATIONS` dict in `backend/app/chatbot.py` was updated to banking-specific recommended actions for all 5 categories. The knowledge base SOPs (`backend/data/knowledge_base/`) were replaced with 5 banking SOPs covering:
- Cards Complaints (unauthorized charges, declines, chargebacks, prepaid)
- Accounts Complaints (frozen accounts, overdrafts, wire transfers, ATM)
- Loans Complaints (mortgage servicing, loan modifications, foreclosure, student/auto/payday loans)
- Collections Complaints (FCRA disputes, FDCPA harassment, charge-offs, identity theft)
- Other Banking Complaints (hidden fees, account opening/closure, crypto-adjacent, escalations)

The chatbot does NOT train on `banking_complaints.csv`. The BM25 index was rebuilt over the 5 new SOP files (39 chunks). The chatbot UI label was changed from "AI Chatbot" to "Chatbot".

### Files Changed

| File | Change |
|---|---|
| `backend/data/clean_banking_dataset.py` | **New.** Reproducible cleaning script. |
| `backend/data/banking_complaints.csv` | **New.** 12,000 clean banking complaints. |
| `backend/data/raw/consumer_complaints.csv` | **New (raw — untouched).** Original CFPB download. |
| `backend/data/EDA.md` | **New.** Full EDA documentation. |
| `backend/app/categories.py` | 5 banking categories replace 5 telecom categories. |
| `backend/app/classify.py` | Banking keyword map; DEFAULT_CATEGORY = "Other banking". |
| `backend/app/sentiment.py` | Banking urgency terms added (overdraft, foreclosure, fraud, etc.). |
| `backend/app/chatbot.py` | Banking RECOMMENDATIONS dict; fallback key "Other banking". |
| `backend/app/main.py` | `_jaccard_similarity`, `_is_repeat_same_day`, repeat rule in `create_complaint`; category removed from `ComplaintIn`. |
| `backend/app/baseline_cache.py` | Points at banking CSV; updated column names. |
| `backend/app/dataset_seed.py` | Points at banking CSV; reads `complaint` and `category` columns. |
| `backend/data/load_dataset.py` | Supports banking CSV schema; docstring updated. |
| `backend/app/ml/train_classifier.py` | Uses official CFPB labels; prefers banking CSV. |
| `backend/app/ml/train_priority.py` | Prefers banking CSV; correct threshold documentation. |
| `backend/data/knowledge_base/` | 5 banking SOPs replace 5 telecom SOPs; BM25 rebuilt. |
| `frontend/activities.html` | Category `<select>` removed; table headers updated. |
| `frontend/script.js` | Category removed from POST body and table renderers; priority badge added. |
| `frontend/admin-dashboard.js` | Banking category names in all 3 hardcoded arrays. |
| `frontend/admin-chatbot.html` | "AI Chatbot" → "Chatbot". |
| `frontend/admin-chatbot.js` | Badge text updated. |
| `backend/tests/test_classify.py` | Banking category assertions. |
| `backend/tests/test_priority.py` | Banking text + same-day repeat tests. |
| `backend/tests/test_api.py` | Banking categories; no-client-category test; repeat-day test. |
| `backend/tests/test_analytics.py` | Banking category names in fixtures. |
| `backend/tests/test_knowledge_base.py` | Banking SOP filenames; BM25 keyword test. |
| `backend/tests/test_ml_models.py` | Banking category assertions; dataset_is_banking check. |
| `docs/GETTING_STARTED.md` | Rewritten to match actual implementation. |
